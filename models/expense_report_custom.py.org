    ...
    def _get_rekap_data(self, date_from, date_to, employee_id, report_type):
        ...
        if report_type == 'sum':
            ...
            data = self.env['hr.expense.sheet'].search(
                ['&', '&', ('accounting_date', '>=', date_from), ('accounting_date', '<=', date_to), ('state', 'in', ['post','done'])])
            ...
            data = self.env['hr.expense'].search(
                ['&', '&', ('date', '>=', date_from), ('date', '<=', date_to), ('state', 'in', ['post','done'])])

            if employee_id:
                data = self.env['hr.expense'].search(['&', '&', '&', ('date', '>=', date_from),
                            ('date', '<=', date_to), ('employee_id', '=', employee_id), ('state', 'in', ['post','done'])])
